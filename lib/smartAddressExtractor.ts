...
// Função para calcular score de similaridade entre textos
...
function calculateAddressScore(text: string, pattern: any, match: RegExpExecArray): number {
  ...
  for (const pattern of ADDRESS_PATTERNS) {
    ...
    while ((match = pattern.pattern.exec(normalizedText)) !== null) {
      ...
      if (extractedText.length >= 10 && extractedText.length <= 200) {
        const score = calculateAddressScore(extractedText, pattern, match);
